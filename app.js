...
const SPECIAL_ABILITIES = {
  "Playmaker": {positions:["DMC","MR","ML","MC","AMC"], description:"Helps transition the ball smoothly from defense to attack, increasing assist potential and controlling the pace."},
  "One-on-One Scorer": {positions:["AMR","AML","AMC","ST"], description:"Gives your striker an advantage when facing the goalkeeper alone in scoring situations."},
  "One-on-One Stopper": {positions:["GK"], description:"Improves the goalkeeper's performance when an opponent breaks through the defensive line for a direct one-on-one shot."},
  "Penalty Kick Specialist": {positions:["DMC","MR","ML","MC","AMR","AML","AMC","ST"], description:"Increases accuracy and success rates when taking penalty kicks."},
  "Penalty Kick Stopper": {positions:["GK"], description:"Boosts the goalkeeper's chance of saving a penalty kick."},
  "Free Kick Specialist": {positions:["DMC","MR","ML","MC","AMR","AML","AMC","ST"], description:"Increases the accuracy and danger level of direct free kick shots on goal."},
  "Corner Specialist": {positions:["DMC","MR","ML","MC","AMR","AML","AMC","ST"], description:"Improves the precision of corner kicks, leading to higher chance creation for headers."},
  "Defensive Wall": {positions:["DR","DL","DC","DMR","DML"], description:"Helps block long-range shots and direct free kicks effectively when positioned properly."},
  "Aerial Defender": {positions:["DR","DL","DC","DMR","DML"], description:"Enhances defensive capability during high balls, crosses, and aerial duels inside the box."},
  "Shadow Striker": {positions:["DMC","MR","ML","MC","AMR","AML","AMC","ST"], description:"Allows a midfielder or secondary attacker to make surprise late runs into the penalty area to score."},
  "Dribbler": {positions:["DMC","MR","ML","MC","AMR","AML","AMC","ST"], description:"Gives the player better control to beat defenders in tight spaces using individual skill."}
};

const PLAYSTYLES = {
  "Goal Poacher": {positions:["ST"], description:"Focuses heavily on finding space in the box and finishing chances with high efficiency."},
  "False Nine": {positions:["ST","AMC"], description:"Drops deep to link up play, pulling opposing central defenders out of position and switching between an attacker and a playmaker."},
  "Target Man": {positions:["ST"], description:"Uses physical presence to hold up the ball, win aerial duels, and bring incoming wingers or midfielders into the attack."},
  "Inside Forward": {positions:["AML","AMR"], description:"Cuts inside from the flanks toward the penalty area to shoot or create high-danger scoring opportunities."},
  "False Winger": {positions:["ML","MR","AML","AMR"], description:"Operates wide but tucks inside or delays crosses to confuse fullbacks and create central passing lanes."},
  "Box-to-Box": {positions:["MC"], description:"Covers immense ground, contributing actively to both defensive recovery and offensive support."},
  "Regista": {positions:["MC","DMC"], description:"Acts as a deep-lying playmaker who dictates the tempo and launches attacks with precise long or short passes from deep areas."},
  "Playmaker": {positions:["MC","AMC"], description:"Unlocks defences in the final third with visionary passing and decisive final-ball execution."},
  "Ball Winner": {positions:["DMC","MC"], description:"Aggressively presses opponents, intercepts passes, and wins tackles to turn over possession."},
  "Anchor Man": {positions:["DMC"], description:"Holds a disciplined position directly in front of the center-backs to shut down central threats."},
  "Mezzala": {positions:["MC"], description:"Drifts into half-spaces during attacks to overload wide-central areas while maintaining central stability."},
  "No-Nonsense DC": {positions:["DC"], description:"Clears the ball immediately under pressure without taking risks or playing fancy passes."},
  "Stopper": {positions:["DC"], description:"Steps up aggressively to challenge attackers, excelling in tight marking and heavy tackling inside the box."},
  "Ball-Playing DC": {positions:["DC"], description:"Combines defensive resilience with clean passing technique to initiate plays cleanly from the backline."},
  "Full-Back / Wing-Back": {positions:["DL","DR"], description:"Patrols the flanks, supporting defensive shapes while overlapping to provide width and crosses."},
  "Sweeper Keeper": {positions:["GK"], description:"Rushes off the line to sweep up through-balls and neutralize counter-attacks early."}
};
...
function availablePlaystylesFor(position){
  const pos=normalisePosition(position);
  return Object.entries(PLAYSTYLES).filter(([,p])=>p.positions.includes(pos));
}
...
function renderPlaystylePicker(containerId, position, selected='', disabled=false){
  ...
  const available=availablePlaystylesFor(pos);
  if(!available.length){
    el.innerHTML=pos ? `<div class="ability-empty">No playstyles are available for <b>${escapeHtml(pos)}</b>. Choose a playstyle only from this position's list.</div>` : '<div class="ability-empty">Select a player position to see compatible playstyles.</div>';
    const countId=containerId==='addPlaystylePicker'?'addPlaystyleCount':null; if(countId) document.getElementById(countId).textContent='0 / 1';
    return;
  }
  const validSelected=available.some(([name])=>name===chosen)?chosen:'';
  el.innerHTML=available.map(([name,p])=>{
    const checked=validSelected===name;
  ...
  el.querySelectorAll('input[data-playstyle]').forEach(box=>box.addEventListener('change',update));
  update();
}

function getSelectedPlaystyle(containerId){
  return document.querySelector(`#${containerId} input[data-playstyle]:checked`)?.getAttribute('data-playstyle') || '';
}

function renderPlaystyleChip(containerId, playstyle){
  const el=document.getElementById(containerId); if(!el) return;
  const value=normalisePlaystyle({playstyle});
  el.innerHTML=value ? `<span class="playstyle-chip">${escapeHtml(value)}</span>` : '<div class="ability-empty">No playstyle assigned.</div>';
}
...
function renderAbilityPicker(containerId, position, selected=[], disabled=false){
  ...
  const available=Object.entries(SPECIAL_ABILITIES).filter(([,a])=>a.positions.includes(position));
  if(!available.length){ el.innerHTML='<div class="ability-empty">No special abilities are available for this position.</div>'; return; }
  ...
  const update=()=>{
    const boxes=[...el.querySelectorAll('input[data-ability]')];
    ...
    const countId=containerId==='addAbilityPicker'?'addAbilityCount':null;
    if(countId) document.getElementById(countId).textContent=`${checked.length} / 2`;
  };
  el.querySelectorAll('input[data-ability]').forEach(box=>box.addEventListener('change',update));
  update();
}

function getSelectedAbilities(containerId){
  return [...document.querySelectorAll(`#${containerId} input[data-ability]:checked`)].map(x=>x.getAttribute('data-ability')).slice(0,2);
}

function renderAbilityChips(containerId, abilities){
  const el=document.getElementById(containerId); if(!el) return;
  const list=normaliseAbilities({specialAbilities:abilities});
  el.innerHTML=list.length ? list.map(a=>`<span class="ability-chip">${escapeHtml(a)}</span>`).join('') : '<div class="ability-empty">No special abilities assigned.</div>';
}


let currentPosition = "";
let currentPlayerKey = null;
let trainingPlayerKey = null;
let editingPlayerKey = null;
let profileEditMode = false;
let addPlayerSkills = {};
let pendingPlayerImage = "";
...
async function renderSquadGrid(){
  ...
  if(!container) return;
  const players = await getAllPlayers();
  const positionOrder = {GK:1,DL:2,DC:3,DR:4,DML:5,DMC:6,DMR:7,ML:8,MC:9,MR:10,AML:11,AMC:12,AMR:13,ST:14};
  players.sort((a,b)=>{
...
async function scanImage(dataUrl){
  ...
  try{
    ...
    const playstyleStillValid=availablePlaystylesFor(currentPosition).some(([name])=>name===preservedPlaystyle);
    renderPlaystylePicker('addPlaystylePicker', currentPosition, playstyleStillValid?preservedPlaystyle:'', false);
...
function clearPlayerForm(){
  editingPlayerKey=null; currentPosition=''; addPlayerSkills={}; pendingPlayerImage='';
...
// ============================================================
const PAGES = ['dashboard','squad','add-player','training','player','formation','mentors','tactics','settings'];
...
function navigateBackFallback(){
  const fallback={player:'squad','training':'squad','add-player':'squad',squad:'dashboard',formation:'dashboard',mentors:'dashboard',tactics:'dashboard',settings:'dashboard'};
...
if(navDrawerVersion) navDrawerVersion.textContent = 'v2.10.0';
...
async function savePlayerRecordByKey(key, changes){
  ...
  const updated={...data,...changes,updatedAt:new Date().toISOString(),specialAbilities:normaliseAbilities(changes),playstyle:normalisePlaystyle(changes),specialAbility:''};
...
function renderProfileSkills(data, editable=false){
  ...
  const position=data.position||''; const white=POSITION_WHITE[position]||[]; const pool=skillsFor(position);
...
async function openPlayerProfile(key,pushHistory=true){
  ...
  document.getElementById('profileMeta').textContent=roleCategoryOf(data.position)+(data.updatedAt?' · updated '+new Date(data.updatedAt).toLocaleDateString():'');
...
if(profilePositionEl) profilePositionEl.addEventListener('change',()=>{
  ...
  const current=getSelectedPlaystyle('profilePlaystylePicker');
  const valid=availablePlaystylesFor(position).some(([name])=>name===current);
  renderPlaystylePicker('profilePlaystylePicker',position,valid?current:'',false);
});
...
renderDashboard();
renderSquadGrid();
let initialPage='dashboard'; let initialPlayerKey=null;
try{initialPage=localStorage.getItem('te:lastPage')||'dashboard';initialPlayerKey=localStorage.getItem('te:lastPlayerKey')||null;}catch(_){}
if(initialPlayerKey) currentPlayerKey=initialPlayerKey;
if(location.hash){const hp=location.hash.slice(1).split('?')[0];if(PAGES.includes(hp))initialPage=hp;}
if(initialPage==='player' && initialPlayerKey) openPlayerProfile(initialPlayerKey,false);
else showPage(PAGES.includes(initialPage)?initialPage:'dashboard',false);
...
const PLAYSTYLE_BONUS = {
  'Goal Poacher': ['ST'], 'False Nine':['ST','AMC'], 'Target Man':['ST'], 'Inside Forward':['AML','AMR'], 'False Winger':['ML','MR','AML','AMR'],
  'Box-to-Box':['MC'], 'Regista':['MC','DMC'], 'Playmaker':['MC','AMC'], 'Ball Winner':['DMC','MC'], 'Anchor Man':['DMC'], 'Mezzala':['MC'],
  'No-Nonsense DC':['DC'], 'Stopper':['DC'], 'Ball-Playing DC':['DC'], 'Full-Back / Wing-Back':['DL','DR'], 'Sweeper Keeper':['GK']
};
const ABILITY_BONUS = {
  'One-on-One Stopper':['GK'],'Penalty Kick Stopper':['GK'],'Defensive Wall':['DR','DL','DC','DMR','DML'],'Aerial Defender':['DR','DL','DC','DMR','DML'],
  'Playmaker':['DMC','MR','ML','MC','AMC'],'One-on-One Scorer':['AMR','AML','AMC','ST'],'Penalty Kick Specialist':['DMC','MR','ML','MC','AMR','AML','AMC','ST'],
  'Free Kick Specialist':['DMC','MR','ML','MC','AMR','AML','AMC','ST'],'Corner Specialist':['DMC','MR','ML','MC','AMR','AML','AMC','ST'],'Shadow Striker':['AMR','AML','AMC','ST'],'Dribbler':['MR','ML','MC','AMR','AML','AMC','ST']
};

function formationPositionFit(playerPos, slot){
  const p=normalisePosition(playerPos), s=normalisePosition(slot);
  if(p===s) return 1;
  ...
  return (adjacent[p]||[]).includes(s)?0.55:0.12;
}

function roleSkillScore(player, slot){
  const s=player.skills||{}; const white=POSITION_WHITE[player.position]||[]; const keyBySlot={
  ...
  const keys=keyBySlot[slot]||white; const vals=keys.map(k=>Number(s[k]||0)).filter(v=>v>0); if(!vals.length) return 0;
  return vals.reduce((a,b)=>a+b,0)/vals.length;
}

function playerSlotScore(player,slot){
  const fit=formationPositionFit(player.position,slot);
  const ovr=Number(player.ovr||0);
  const skill=roleSkillScore(player,slot);
  const style=normalisePlaystyle(player);
  const abilities=normaliseAbilities(player);
  const styleBonus=(style && (PLAYSTYLE_BONUS[style]||[]).includes(slot))?12:0;
  const abilityBonus=abilities.reduce((sum,a)=>sum+((ABILITY_BONUS[a]||[]).includes(slot)?4:0),0);
  const positionBonus=fit===1?180:fit===0.55?25:0;
  return positionBonus + fit*(ovr*0.44+skill*0.38)+styleBonus+abilityBonus;
}

function assignFormation(players, formation){
  const remaining=[...players];
  const assignments=new Array(formation.slots.length);
  const slotOrder=formation.slots.map((slot,index)=>({slot,index,exactCandidates:players.filter(p=>normalisePosition(p.position)===normalisePosition(slot)).length}))
    .sort((a,b)=>a.exactCandidates-b.exactCandidates || a.index-b.index);

  // Fill the rarest natural positions first. This prevents a versatile player
  // from taking a slot that only one specialist can fill, which was a major
  // cause of strange out-of-position XIs on larger squads.
  for(const item of slotOrder){
    let best=null,bestScore=-Infinity,bestIndex=-1;
    remaining.forEach((p,i)=>{
      const score=playerSlotScore(p,item.slot);
      if(score>bestScore){best=p;bestScore=score;bestIndex=i;}
    });
    if(best){assignments[item.index]={player:best,slot:item.slot,score:bestScore};remaining.splice(bestIndex,1);}
  }
  const chosen=assignments.filter(Boolean);
  const total=chosen.reduce((sum,x)=>sum+x.score,0);
  const exact=chosen.filter(x=>normalisePosition(x.player.position)===normalisePosition(x.slot)).length;
...
function chooseBestFormation(players){
  ...
  return FORMATIONS.map(f=>({formation:f,...assignFormation(players,f)})).sort((a,b)=>b.total-a.total)[0];
}
...
function renderFormation(result){
  ...
  // Coordinates are deliberately spread over separate horizontal/vertical lines.
  // These follow the normal Top Eleven shape: GK deepest, back line in front,
  // midfield between the boxes, and attackers highest. Same-role players get
  // their own lane rather than being stacked on top of each other.
  const layouts={
    '4-4-2': [
      ['GK',50,91],['DL',15,76],['DC',40,79],['DC',60,79],['DR',85,76],
      ['ML',15,54],['MC',39,54],['MC',61,54],['MR',85,54],['ST',40,25],['ST',60,25]
    ],
    '4-3-3': [
      ['GK',50,91],['DL',15,76],['DC',40,79],['DC',60,79],['DR',85,76],
      ['MC',25,53],['MC',50,55],['MC',75,53],['AML',19,29],['ST',50,18],['AMR',81,29]
    ],
    '4-2-3-1': [
      ['GK',50,91],['DL',15,76],['DC',40,79],['DC',60,79],['DR',85,76],
      ['DMC',36,62],['DMC',64,62],['AML',18,36],['AMC',50,35],['AMR',82,36],['ST',50,15]
    ],
    '3-5-2': [
      ['GK',50,91],['DC',25,78],['DC',50,80],['DC',75,78],
      ['ML',14,55],['MC',37,55],['MC',50,58],['MC',63,55],['MR',86,55],['ST',40,24],['ST',60,24]
    ],
    '4-5-1': [
      ['GK',50,91],['DL',15,76],['DC',40,79],['DC',60,79],['DR',85,76],
      ['DMC',50,65],['ML',14,49],['MC',39,50],['MR',86,49],['AMC',50,32],['ST',50,15]
    ],
    '4-1-2-1-2': [
      ['GK',50,91],['DL',15,76],['DC',40,79],['DC',60,79],['DR',85,76],
      ['DMC',50,65],['MC',36,51],['MC',64,51],['AMC',50,34],['ST',40,16],['ST',60,16]
    ]
  };

  const layout=layouts[result.formation.name]||[];
  result.chosen.forEach(({player,slot},index)=>{
    const spec=layout[index] || [slot,50,50];
    const el=document.createElement('div'); el.className='pitch-player';
    el.style.left=spec[1]+'%'; el.style.top=spec[2]+'%';
    const lastName=String(player.name||'Player').trim().split(/\s+/).pop()||'Player';
    el.title=`${player.name} · ${slot} · OVR ${player.ovr||'—'}`;
    el.innerHTML=`<div class="pitch-player-dot">${player.profileImage?`<img src="${player.profileImage}" alt="">`:`<span>${escapeHtml(slot)}</span>`}</div><div class="pitch-player-name">${escapeHtml(lastName)}</div>`;
    el.addEventListener('click',()=>openPlayerProfile(player.key)); pitch.appendChild(el);
  });
  bench.innerHTML=result.remaining.slice(0,8).map(p=>`<div class="bench-card"><b>${escapeHtml(p.name||'Player')}</b><br><span>${escapeHtml(p.position||'?')} · OVR ${escapeHtml(p.ovr||'—')}${normalisePlaystyle(p)?' · '+escapeHtml(normalisePlaystyle(p)):''}</span></div>`).join('');
}

async function runTeamSelect(){
  const players=await getAllPlayers(); const result=chooseBestFormation(players);
  if(result.error){document.getElementById('formationName').textContent='Not enough players';document.getElementById('formationSummary').textContent=result.error;showToast(result.error,'err');return;}
  document.getElementById('formationName').textContent=result.formation.name;
  document.getElementById('formationSummary').textContent=`${result.exact}/11 natural-position picks · weighted team score ${Math.round(result.total)} · playstyles and abilities included`;
  renderFormation(result);
  try{await window.storage.set('formation:last',JSON.stringify({formation:result.formation.name,chosen:result.chosen.map(x=>({key:x.player.key,slot:x.slot})),updatedAt:new Date().toISOString()}),false);}catch(_){}
}
